...
skipModule :: ModuleName -> Bool
skipModule (ModuleName name) =
  -- Openstack module are renamed without the os_ prefix
  not (Text.isPrefixOf "openstack.cloud.os_" name)
...
-- | Helper function to create record dictionary
mkMap :: [(Text, a)] -> D.Map Text a
mkMap = D.fromList . L.sortOn fst
...
dhallCollectionPackage xs =
  [ ("Type.dhall", D.Record . mkMap $ L.foldl goType [] xs),
    ("default.dhall", D.RecordLit . mkMap $ L.foldl goDefault [] xs)
  ]
  where
  ...
  where
    ...
    getModuleList ansibleDoc = do
      ...
      case decodeModuleList list of
        ...
        Right moduleList -> pure $ filter skipModule $ filter (not . flip elem skipModules) moduleList
