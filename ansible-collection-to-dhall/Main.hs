...
filterModules :: Maybe Text -> Bool -> [ModuleName] -> [ModuleName]
filterModules prefix onlyBuiltin = L.foldl go []
  where
    go :: [ModuleName] -> ModuleName -> [ModuleName]
    go acc (ModuleName name) =
      let keep = case (prefix, onlyBuiltin) of
            (Just "all", _) -> True
            (Just prefix', False) -> Text.isPrefixOf prefix' name
            (Nothing, True) -> case Text.find ('.' ==) name of
              Just _ -> False
              Nothing -> True
